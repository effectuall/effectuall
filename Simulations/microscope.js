...
const parts = await (await fetch('files/microscopeParts.json')).json();;
// //global variables
...
function init() {
    ...
    createCamera(45, -20, 10, 10, new THREE.Vector3(0, -20, 0));

    createLights();
    createRenderer();
    createControls(2, 0, -2.5);
...
function labelsAction() {
    if (labelDisplay) {
        ...
        for (let i = 0; i < raylabels.length; i++) {
            raylabels[i].visible = true;
        }
    } else if (!iRay) {

        for (let i = 0; i < raylabels.length; i++) {
            raylabels[i].visible = false;
        }
    } else {
        for (let i = 0; i < labels.length; i++) {
